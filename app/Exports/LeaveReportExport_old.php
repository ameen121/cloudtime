...
class LeaveReportExport implements FromCollection,WithHeadings,ShouldAutoSize
{
    ...
    {
        return ['Staff Id', 'Staff Name', 'Branch', 'From Roster Date and Time', 'To Roster Date and Time','Time1','Time2','Reason','Total Working Houre'];
    }
    ...
    // public function view(): View
    {
        ...
        $data=[];
    ...
    
                    {
                        ...
                        $array['reason']=DB::table('attandance_setup')->where('company_id',$empldetail->company_id)->whereDate('created_at','>=',$startDate)->whereDate('created_at','<=',$endDate)->get()->pluck('name')->toArray();
                       
                        $array['reasons_time']=DB::table('attendances')
                     ...
                     if($array['reasons_time']->count()>0){
                        foreach($array['reasons_time'] as $time){
