...
class LoginController extends FrontBaseController
{
    ...
    */
    ...
    protected function validateLogin(\Illuminate\Http\Request $request)
    {
      ...
      $role= DB::table('role_user')->where('user_id',$user->id)->get()->pluck('role_id')->toArray();
  
     $rolesUser= DB::table('roles')->whereIn('id',$role)->get();
     
$tyarray=[];
     foreach($rolesUser as $value){
        $rolesUser1['type']= $value->name;
        $rolesUser1['id']= $value->id;
     
        $tyarray=$rolesUser1;

     }
     
     if(in_array('Branch Manager',$tyarray)){
        $rolesUser2 = DB::table('roles')->whereIn('id',$role)->first();
      
    }
     $employeedetail= DB::table('employee_details')->where('user_id',$user->id)->first();
    ...
    // dd($tyarray);
        ...
        $user_data = [
            ...
            // "type" =>$tyarray['type'],
            "department_id" => $employeedetail ? $employeedetail->department_id : null,
            "type" => isset($tyarray['type']) ? $tyarray['type'] : null,
            "user_id" =>$user->id,
