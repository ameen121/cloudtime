...
class User extends BaseModel implements AuthenticatableContract, CanResetPasswordContract
{
    ...
    {
        return $this->hasMany(Rota::class, 'user_id');
    }
    ...
    public static function allEmployees3($id)
    {
        $users = User::with('employeeDetail')->withoutGlobalScope('active')->join('role_user', 'role_user.user_id', '=', 'users.id')
            ->join('roles', 'roles.id', '=', 'role_user.role_id')
            ->select('users.id', 'users.name', 'users.email', 'users.status', 'users.email_notifications', 'users.created_at', 'users.image', 'users.mobile', 'users.country_id')
        ...
        $users->orderBy('users.name', 'asc');
        $users->groupBy('users.id');
        return $users->get()->where('employeeDetail.department_id',$id);
    }
     public static function allEmployees5($id)
    {
        $users = User::with('employeeDetail')->withoutGlobalScope('active')->join('role_user', 'role_user.user_id', '=', 'users.id')
            ->join('roles', 'roles.id', '=', 'role_user.role_id')
    ...
    public static function allEmployees1($exceptId = null)
    {
        $users = User::with('employeeDetail')->withoutGlobalScope('active')->join('role_user', 'role_user.user_id', '=', 'users.id')
            ->join('roles', 'roles.id', '=', 'role_user.role_id')
            ->select('users.id', 'users.name', 'users.email', 'users.status', 'users.email_notifications', 'users.created_at', 'users.image', 'users.mobile', 'users.country_id')
    ...
    public static function allEmployees2($id)
    {

        $users = User::with('employeeDetail')->withoutGlobalScope('active')->join('role_user', 'role_user.user_id', '=', 'users.id')
            ->join('roles', 'roles.id', '=', 'role_user.role_id')
            ->join('employee_details', 'employee_details.user_id', '=', 'users.id')
            ->select('employee_details.department_id','users.id', 'users.name', 'users.email', 'users.status', 'users.email_notifications', 'users.created_at', 'users.image', 'users.mobile', 'users.country_id')
            ->where('roles.name', '<>', 'client')
            ->where('employee_details.department_id', $id);
