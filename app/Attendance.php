...
class Attendance extends BaseModel
{
    ...
    protected $appends = ['clock_in_date','time_zone_date'];
    ...
    public function getClockInDateAttribute()
    {
      ...
      public function getTimeZoneDateAttribute()
    {
       
        $global = Company::where('id', Auth::user()->company_id)->first();
        $timestamp = strtotime($this->clock_in_time);
        $dt = new \DateTime("now", new \DateTimeZone($global->timezone)); 
    ...
    public static function attendanceDate($date)
    {
      ...
      public static function attendanceDateFilterByTimeZone($date)
    {
    ...
    public static function userAttendanceByDate($startDate, $endDate, $userId)
    {
          ...
          // dd($userId);
        // dd(date('Y-m-d',strtotime($endDate)));
         ...
         if(date('Y-m-d',strtotime($startDate))==date('Y-m-d',strtotime($endDate)))
        {
           ...
           if($currentDate<$startDate){
             ...
             $startDate=date('Y-m-d h:i:s a', strtotime('-1 day', strtotime( $startDate )));
          
            return  Attendance::join('users', 'users.id', '=', 'attendances.user_id')
                    ->where(DB::raw('DATE(attendances.clock_in_time)'), '>=', date('Y-m-d',strtotime($startDate)))
       ...
       else
       {
        ...
        //   dd($startDate.'-'.$endDate);
           return Attendance::join('users', 'users.id', '=', 'attendances.user_id')
               ...
               ->where('attendances.user_id', '=', $userId)
               ->orderBy('attendances.id', 'asc')
                ->select('attendances.*','users.*', 'attendances.id as aId',DB::raw('DATE(attendances.clock_in_time) as clock_in_date')  )
               ->get();
