...
@section('content')
    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-header">
                <h3 class="card-title">Roster <b>({{ $startDate1 }} To {{ $endDate1 }})</b>         |<b>  Title : {{$departmentsrequested!=null? $departmentsrequested->team_name:'All Branch'}}</b></h3>
                {{--                <div class="float-right">--}}
                                    ...
                                    @if($check && count($check)>0)
                                        ...
                                        @php
                                         $checkDate=DB::table('rotas')->where('date',$time['date'])->where('user_id', $staff->id)->get()->toArray();
                                            // $checkDate=$check->where('date',$time['date'])->get();
