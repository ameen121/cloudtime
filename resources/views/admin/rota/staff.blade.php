...
@section('content')
                ...
                <form method="GET" action="{{ route('admin.rota.search') }}">
                    <div class="row">
                        ...
                        <div class="col-md-3 department">
                            <div class="form-group">
                                <select class="form-control" name="department" class="">
    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-header">
                @php
               $username= DB::table('users')->where('id', $staff_user)->first();
            //    dd($username);
                @endphp
                <h3 class="card-title"> <b>({{ $startDate1 }} To {{ $endDate1 }}) </b>    |<b>  Title : {{$username!=null? 'Staff('.$username->name.')':''}}</b></h3>
                {{--                <div class="float-right">--}}
 ...
 $color="b1bc3c";
                                        ...
                                        <td class="text-center text-uppercase profile-pic" @if($checkDate) style="background-color: #{{ $color }}" @endif>
                                                                                        ...
                                                                                        @if($checkDate)
                                                                                            <div class="delete"><a href="{{ route('admin.rota.delete',$checkDate->id) }}" onclick="return confirm('Are you sure you want to delete?');"><i class="fa fa-trash"></i></a></div>
                                                                                            {{ date('g:i A',strtotime($checkDate->start_time)) }} - {{ date('g:i A',strtotime($checkDate->end_time))  }}

                                                <div class="edit"><a onclick="showAdd1({{$checkDate->id}})"><i
